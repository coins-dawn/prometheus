...
def dijkstra(
    ...
    for start in start_candidates:
        heapq.heappush(pq, (start_distances[start] / WALK_SPEED, start, "walk"))
    ...
    while pq:
        ...
        if node in goal_candidates:
            ...
            for i in range(len(path) - 1):
                ...
                edge_list.append({"org_node": org, "dst_node": dst, "type": mode})
...
class PtransSearcher:
    ...
    def _load_trip_pairs(
        self, trip_pairs_file_path: str
    ...
    def search(self, input: PtransSearchInput) -> PtransSearchOutput:
        ...
        start_2_first_stop_distance = int(
            haversine(
                input.start.lat,
                input.start.lon,
                first_stop_coord[0],
        ...
        for section in best_path:
            ...
            # いったんweekdayにきめうち
            time_table = (
                self.trip_pairs.get((org_node, dst_node), {})
                .get("weekday", {})
                .get("time_table", [])
            )
            ...
            sections.append(
                PtransOutputSection(
                    ...
                    name=self.trip_pairs.get((org_node, dst_node), {})
                    .get("weekday", {})
                    .get("name", ""),
                )
            )
