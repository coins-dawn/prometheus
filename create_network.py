...
def write_csv_and_binary(nodes, ways, prefix):
    """ CSV と バイナリ両方にデータを保存 """
    base_path = "convert/output"
    nodes_csv = f"{base_path}/{prefix}_nodes.csv"
    ways_csv = f"{base_path}/{prefix}_ways.csv"
    nodes_bin = f"{base_path}/{prefix}_nodes.bin"
    ways_bin = f"{base_path}/{prefix}_ways.bin"
    ...
    with open(nodes_bin, mode="wb") as f:
        f.write(struct.pack("I", len(nodes)))
        for node_id, (lat, lon, mesh_code) in nodes.items():
            f.write(struct.pack("IddI", node_id, lat, lon, mesh_code))  

    with open(ways_bin, mode="wb") as f:
        f.write(struct.pack("I", len(ways)))
        for way in ways:
            f.write(struct.pack("IIf", way["start_node"], way["end_node"], way["length_m"]))
...
pbf_file = "convert/input/chubu-latest-filtered.osm.pbf"
...
write_csv_and_binary(car_nodes, car_ways, "car")
write_csv_and_binary(walk_nodes, walk_ways, "walk")

print("CSV & バイナリファイルを出力しました: car_nodes.csv, car_ways.csv, car_nodes.bin, car_ways.bin, walk_nodes.csv, walk_ways.csv, walk_nodes.bin, walk_ways.bin")
